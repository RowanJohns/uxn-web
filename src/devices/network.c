...
void print_stack(void) {
    ...
    // printf("[STACK] wst: ");
    for (i = 0; i < 256; i++) {
        printf("%x ", uxn.wst.dat[i]);
    }
    printf("\n");
    printf("[STACK] wst pointer: %x", uxn.wst.ptr);
    printf("\n");
    /* printf("[STACK] rst: ");
    for (i = 0; i < 256; i++) {
        printf("%x ", uxn.rst.dat[i]);
    }
    printf("\n");
    printf("[STACK] rst pointer: %x", uxn.rst.ptr);
    printf("\n"); */
}
...
void network_deo(Uint8 addr) {
    ...
    /* printf("[DEBUG DEO] dev[0xa8]: %d\n", uxn.dev[0xa8]);
    printf("[DEBUG DEO] dev[0xa9]: %d\n", uxn.dev[0xa9]); */
    ...
    switch (addr & 0x0f) {
        ...
        case 0x0b: /* read address (low byte) - trigger read */
            {   
                ...
                /* printf("[DEBUG] Address: %d, dev[address]: %d, dev[address-1]: %d\n", addr, value, uxn.dev[addr-1]); */
